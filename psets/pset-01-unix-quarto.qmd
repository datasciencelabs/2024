...
After finishing the homwork you are to turn in all the code
...
3. Write a Quarto document that defines variables $a=1, b=-1, c=-2$
and print out the solutions to $f(x) = ax^2+bx+c=0$. 
Do not report complex solutions, only real numbers. 
...
5. Create a directory called `docs`. Use the command `quarto render` to create a PDF and save it to the `docs` directory. Show use the command you typed:
